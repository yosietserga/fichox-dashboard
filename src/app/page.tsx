...
export default function Home() {
  const [tab, setTab] = useState<'plan' | 'brand'>('brand')

  return (
    <div className="min-h-screen flex flex-col bg-white">
      {/* Top tab switcher */}
      <div className="sticky top-0 z-[60] border-b border-blue-100/70 bg-white/90 backdrop-blur-md">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 h-12 sm:h-13 flex items-center justify-center gap-1">
          <button
            onClick={() => setTab('plan')}
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="flex-1"
        >
          {tab === 'plan' ? <BusinessPlanView /> : <BrandingManualView />}
        </motion.div>
      </AnimatePresence>
    </div>
  )
